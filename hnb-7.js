...
async function main() {
  ...
  for (const result of categoryResults) {
    if (result.success && result.offers.length > 0) {
      const filename = `${result.category.toLowerCase().replace(/\s+/g, '_')}_v6.json`;
      fs.writeFileSync(
  ...
  fs.writeFileSync(
    path.join(outputDir, 'hnb_all_v6.json'),
    JSON.stringify({
      processedAt: new Date().toISOString(),
      stats,
  ...
  fs.writeFileSync(
    path.join(outputDir, 'hnb_validity_rows_v6.json'),
    JSON.stringify({
      processedAt: new Date().toISOString(),
  ...
  console.log(`  ✓ hnb_validity_rows_v6.json (${validityRows.length} rows - DB import ready)`);

  // 4. Raw data
  fs.writeFileSync(
    path.join(outputDir, 'hnb_raw_v6.json'),
    JSON.stringify({
  ...
  console.log(`  ✓ hnb_raw_v6.json (with raw data)`);
