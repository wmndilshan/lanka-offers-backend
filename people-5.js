...
class PeriodParser {
  ...
  static parse(validityRaw) {
    ...
    const engine = PeriodEngine.parse(validityRaw, { defaultPeriodType: 'offer' });
    if (engine.length > 0) {
...
class OfferValidity {
  ...
  isActiveOn(dateStr) {
    ...
    if (this.recurrence_type === 'specific_weekdays' && this.recurrence_days) {
      ...
      const days = Array.isArray(this.recurrence_days)
        ? this.recurrence_days
        : this.recurrence_days.split(',');
      if (!days.includes(dayName)) return false;
    }

    if (this.recurrence_type === 'specific_dates' && this.recurrence_days) {
      const dates = Array.isArray(this.recurrence_days)
        ? this.recurrence_days
        : this.recurrence_days.split(',');
